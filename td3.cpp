...
double* shrink_array(double* array, int length, int new_size) {
    // create new array and copy the elements from original array
    double* new_array = new double[new_size];
    for (int i = 0; i < new_size; i++)
        new_array[i] = array[i];
...
void merge_telemetry(double **telemetries,
    ...
    for (int i = 0; i < tot_telemetries; i++) {
        for (int j = 0; j < telemetries_sizes[i]; j++) {
            telemetry = append_to_array(telemetries[i][j], telemetry,
    ...
    for (int i = 0; i < telemetry_current_size/3-1; i++) {
        for (int j = 0; j < telemetry_current_size/3-i-1; j++) {
            if (telemetry[3*j] > telemetry[3*(j+1)]) {
                // swap t
                double temp = telemetry[3*j];
                telemetry[3*j] = telemetry[3*(j+1)];
                telemetry[3*(j+1)] = temp;
                // swap x
                temp = telemetry[3*j+1];
                telemetry[3*j+1] = telemetry[3*(j+1)+1];
                telemetry[3*(j+1)+1] = temp;
                // swap y
                temp = telemetry[3*j+2];
                telemetry[3*j+2] = telemetry[3*(j+1)+2];
                telemetry[3*(j+1)+2] = temp;
            }
        }
    }
}
